              ...
              <div class="h-fit sm:w-1/2 sm:max-w-[50vw]" data-scroll data-scroll-sticky data-scroll-target="#fixed-elements">
                <div class="card border w-full shadow-xl rounded-xl">
                  <figure class="px-4 pt-4">
                    <img
                      id="img-preview-admin-mekanismeKeberatan-preview"
                  ...
                  <div class="py-5 pr-5 text-end self-end w-1/2">
                    <h2 class="text-xl text-indigo-500 font-extrabold">MEKANISME KEBERATAN</h2>
                  </div>
                </div>
              </div>
...
<section class="relative mx-5 sm:mx-10 bg-base-100 shadow-lg rounded-lg p-4 sm:p-10 mb-5">
  ...
  <div class="absolute transform translate-x-[1px] translate-y-[190px]">
    <form action="<?= base_url('api/admin/mekanisme-keberatan/delete-image/' . $results['id_mekanisme_keberatan']) ?>" method="POST" class="inline">
      <?= csrf_field() ?>
  ...
  <form id="myForm" action="<?= base_url() ?>/api/admin/mekanisme-keberatan/edit/<?= $results['id_mekanisme_keberatan'] ?>" method="POST" enctype="multipart/form-data">
    ...
    <div class="flex justify-between items-center w-full gap-3">
      <div class="text-md sm:text-xl font-bold">Edit Data Mekanisme Keberatan</div>
      ...
      <div class="w-full">
        <div class="flex flex-col sm:flex-row gap-3 justify-center w-full mb-3">
          <div class="w-full sm:w-8/12">
            ...
            <input id="img-input-admin-mekanismeKeberatan" name="link_gambar_edit" type="file" class="file-input file-input-bordered w-full <?= (isset($errors['link_gambar_edit'])) ? 'input-error' : 'mb-4' ?>" onchange=" previewImgAdminmekanismeKeberatan()" />
          ...
          <div class="w-4/12">
            <div class="relative border bg-neutral w-full mt-7">
              <img id="img-preview-admin-mekanismeKeberatan" class="w-full h-auto" src="<?= base_url() ?>img/standarLayanan/mekanismeKeberatan/<?= $results['link_gambar'] ?? 'img/icon/default-image.jpg' ?>" alt="">
              <div class="absolute bottom-0 left-0 right-0 z-10 h-2/4 bg-gradient-to-t"></div>
            </div>

            <script>
              function previewImgAdminmekanismeKeberatan() {
                const cover = document.querySelector('#img-input-admin-mekanismeKeberatan');
                const imgPreview = document.querySelector('#img-preview-admin-mekanismeKeberatan');
                const imgPreviewDropdown = document.querySelector('#img-preview-admin-mekanismeKeberatan-preview');
        ...
        <div class="flex gap-3 justify-center mb-3 flex-col">
          ...
          <?php endif ?>
          <textarea class="textarea textarea-bordered w-full" placeholder="Mekanisme Keberatan" name="content_edit" id="content">
          <?php if (!empty($results['content'])) : ?>
            <?= $results['content'] ?>
          <?php else : ?>
            <li class="text-xs">-</li>
