  ...
  <div class="modal-box">
    ...
    <div class="py-4">
      ...
      <form action="<?= base_url() ?>/api/admin/regulasi/create" method="post">
        ...
        <input name="link_drive_create" type="text" placeholder="Link Goggle Drive PDF" class="input input-bordered w-full <?= (isset($errors['link_drive_create'])) ? 'input-error' : 'mb-3' ?>" value="<?= old('link_drive_create') ?>" />
        <?php if (isset($errors['link_drive_create'])) : ?>
          <div class="label"><span class="label-text-alt text-error"><?= $errors['link_drive_create'] ?></span></div>
        <?php endif ?>
