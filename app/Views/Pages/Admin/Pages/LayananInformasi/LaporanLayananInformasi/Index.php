              ...
              <div id="customHeight" class="h-fit w-full md:w-1/2" data-scroll data-scroll-sticky data-scroll-target="#fixed-elements">
                <div class="card border w-full shadow-xl rounded-xl">
                  ...
                  <div class="py-5 pr-5 text-end w-full">
                    <h2 class="text-xl md:text-2xl text-indigo-500 font-extrabold">LAPORAN LAYANAN INFORMASI</h2>
                  </div>
                </div>
              </div>
              ...
              <div class="max-w-2xl shadow-xl border rounded-xl w-full">
                ...
                <?php if (!empty($results['content'])) : ?>
                  ...
                  <div class="px-6 md:px-10 py-6 md:py-8">
                    <article class="prose prose-sm md:prose lg:prose-lg prose-headings:text-indigo-500" id="content-preview-laporan-layanan-informasi">
                      <?= $results['content'] ?>
                    </article>
                    <div class="border-t mt-4 pt-4">
                      <h2 class="text-xl md:text-2xl text-indigo-500 font-extrabold text-end">LAPORAN LAYANAN INFORMASI</h2>
                    </div>
                  </div>
                <?php endif; ?>
...
<section class="relative mx-5 sm:mx-10 bg-base-100 shadow-lg rounded-lg p-4 sm:p-10 mb-5">
  ...
  <div class="absolute transform translate-x-[1px] translate-y-[190px]">
    <form action="<?= base_url('api/admin/laporan-layanan-informasi/delete-image/' . $results['id_laporan_layanan_informasi']) ?>" method="POST" class="inline">
      <?= csrf_field() ?>
  ...
  <form id="myForm" action="<?= base_url() ?>/api/admin/laporan-layanan-informasi/edit/<?= $results['id_laporan_layanan_informasi'] ?>" method="POST" enctype="multipart/form-data">

    <!-- Title & Edit Button -->
    <div class="flex justify-between items-center w-full gap-3">
      <div class="text-md sm:text-xl font-bold">Edit Data Laporan Layanan Informasi</div>
      ...
      <div class="w-full">
        <div class="flex flex-col sm:flex-row gap-3 justify-center w-full mb-3">
          <div class="w-full sm:w-8/12">
            ...
            <input id="img-input-admin-laporanLayananInformasi" name="link_gambar_edit" type="file" class="file-input file-input-bordered w-full <?= (isset($errors['link_gambar_edit'])) ? 'input-error' : 'mb-4' ?>" onchange=" previewImgAdminLaporanLayananInformasi()" />
          ...
          <div class="w-4/12">
            <div class="relative border bg-neutral w-full mt-7">
              <img id="img-preview-admin-laporanLayananInformasi" class="w-full h-auto" src="<?= base_url() ?>img/layananInformasi/laporanLayananInformasi/<?= $results['link_gambar'] ?? 'img/icon/default-image.jpg' ?>" alt="">
        ...
        <div class="flex gap-3 justify-center mb-3 flex-col">
          ...
          <?php endif ?>
          <textarea class="textarea textarea-bordered w-full" placeholder="Laporan Layanan Informasi" name="content_edit" id="content">
          <?php if (!empty($results['content'])) : ?>
            <?= $results['content'] ?>
          <?php else : ?>
            <li class="text-xs">-</li>
          ...
          <script>
            ...
            CKEDITOR.replace('content', {
              ...
              uploadUrl: '<?= base_url('/api/admin/laporan-layanan-informasi/upload-image') ?>',
              filebrowserUploadUrl: '<?= base_url('/api/admin/laporan-layanan-informasi/upload-image') ?>',
