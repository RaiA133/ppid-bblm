          ...
          <?php foreach ($results as $result) : ?>
            <tr class="border-b">
              ...
              <td class="">
                ...
                <a class="btn btn-xs btn-neutral w-14 mb-1 lg:xl-0" onclick="editDataInformasiBerkalaJudul<?= esc($result['id_informasi_berkala_judul']) ?>.showModal()">Edit</a>

                <?php if (session()->getFlashdata('openModalEditDataInformasiBerkalaJudul' . esc($result['id_informasi_berkala_judul']))): ?>
                  <script>
                    document.addEventListener("DOMContentLoaded", function() {
                      document.getElementById("editDataInformasiBerkalaJudul<?= esc($result['id_informasi_berkala_judul']) ?>").showModal();
                    });
                  </script>
                <?php endif; ?>

                <dialog id="editDataInformasiBerkalaJudul<?= esc($result['id_informasi_berkala_judul']) ?>" class="modal">
                  <div class="modal-box">
                    <form method="dialog">
                    ...
                    <div class="py-4">
                      ...
                      <form action="<?= base_url() ?>api/admin/informasi-berkala/manage-judul/edit/<?= esc($result['id_informasi_berkala_judul']) ?>" method="post">
                        <?= csrf_field() ?>
                ...
                <form action="<?= base_url() ?>api/admin/informasi-berkala/manage-judul/delete/<?= esc($result['id_informasi_berkala_judul']) ?>" method="POST" class="inline">
                  <?= csrf_field() ?>
